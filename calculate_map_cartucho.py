...
ground_truth_pattern = re.compile(r'^(bus|car|truck):\s*(\d+)$')
tp_fp_pattern = re.compile(r'\(tp:(\d+),\s*fp:(\d+)\)')
...
if os.path.exists(output_path):
    print(f'The output folder {output_path} already exists. Deleting it...')
    shutil.rmtree(output_path)  # Remove the existing folder and its contents

os.makedirs(output_path)  # Create the folder again
...
overall_tp, overall_fp, overall_p = 0, 0, 0
for i, iou_thresh in enumerate(iou_threshes):
    ...
    with open('output/output.txt', 'r',) as f:
        for line in f:
            ...
            if iou_thresh == 0.5:    
              if ground_truth_match := ground_truth_pattern.search(line):
                class_name = ground_truth_match.group(1)
                overall_p += int(ground_truth_match.group(2))

              # Extract TP and FP counts
              if tp_fp_match := tp_fp_pattern.search(line):
                overall_tp += int(tp_fp_match.group(1))
                overall_fp += int(tp_fp_match.group(2))
...
print()
print(f"Precision: {overall_tp/(overall_tp+overall_fp):.3f}")
print(f"Recall: {(overall_tp/overall_p):.3f}")
