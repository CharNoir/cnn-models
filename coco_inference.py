...
def process_dataset(model_path, labels_path, images_path, annotations_path, iou_thresholds):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--images", type=str, required=True, help="Path to the dataset folder in YOLO format")
    parser.add_argument("--annotations", type=str, required=True, help="Path to the dataset folder in YOLO format")
    args = parser.parse_args()

    iou_thresholds = np.arange(0.5, 1.0, 0.05)  # IoU thresholds from 0.50 to 0.95
    process_dataset(args.model, args.labels, args.images, args.annotations, iou_thresholds)
